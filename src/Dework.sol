...
contract Dework{
    uint256 constant public LATE_PENALITY_BPS_PER_DAY = 500;//500/10000 = 5%
    uint256 constant public BPS_DENOMINATOR = 10000; //100%
    ...
    function createJobListing(
        uint256 employerId,
        CreateJobListingInput calldata jobListingInput
        ) external payable onlyAuthourizedEmployer(employerId){
        ...
        function hireFreelancer(uint256 employerId,uint256 freelancerId,uint256 jobId) public onlyAuthourizedEmployer(employerId) validateJobId(employerId,jobId) validateFreelancerId(freelancerId) {
        ...
        function releaseEscrowPayment(uint256 employerId,uint256 jobId,uint256 freelancerId) public onlyAuthourizedEmployer(employerId) validateJobId(employerId,jobId) validateFreelancerId(freelancerId){
              JobListing storage job =  _getJob(employerId,jobId);
